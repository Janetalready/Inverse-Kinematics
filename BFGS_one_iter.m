...
while b==0
    ...
    if iter == 0
        ...
        disp([qs{2,1},qs{1,1}]);
        s = reshape(s, [DoF, 1]);
        dev2 = round(double(end_effectors{2,1}),10)-target;
        dev2 = reshape(dev2, [2, 1]);
        dev1 = round(double(end_effectors{1,1}),10)-target;
    ...
    delta_theta = round(double(delta_theta),10);
    theta_sub = theta_sub + radtodeg(8*delta_theta);
